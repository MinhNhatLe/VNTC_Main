using dotnetstartermvc.Areas.Identity.Models.RoleViewModels;
...
namespace dotnetstartermvc.Areas.Identity.Controllers
{
    ...
    public class RoleController : Controller
    {
        ...
        public async Task<IActionResult> CreateAsync(CreateRoleModel model)
        {
            ...
            if (result.Succeeded)
            {
                StatusMessage = $"Bạn vừa tạo role mới: {model.Name} thành công!";
        ...
        public async Task<IActionResult> DeleteConfirmAsync(string roleid)
        {
            ...
            if (result.Succeeded)
            {
                StatusMessage = $"Bạn vừa xóa: {role.Name} thành công!";
                return RedirectToAction(nameof(Index));
            }
            else
            {
        ...
        public async Task<IActionResult> EditConfirmAsync(string roleid, [Bind("Name")] EditRoleModel model)
        {
            ...
            if (result.Succeeded)
            {
                StatusMessage = $"Bạn vừa đổi tên: {model.Name} thành công!";
                return RedirectToAction(nameof(Index));
            }
            else
            {
